...
ROLE_REQUIREMENTS = {
    "AI/ML Engineer": [
        "Python", "PyTorch", "TensorFlow", "Machine Leaning", "Deep Learning",
         "MLOps", "Scikit-Learn", "NLP", "Computer Vision", "Reinforcement Learning",
        "Hugging Face", "Data Engineering", "Feature Engineering", "AutoML"
    ],
    "Frontend Engineer":[
        "React", "Vue", "Angular", "HTML5", "CSS3", "JavaScript",
         "TypeScript", "Next.js", "Svelte", "Bootstrap",
          "Tailwind CSS", "GraphQL", "Redux", "WebAssembly", "Three.js", 
          "Performance Optimization"
    ],
    "Backend Engineer":[ "Python", "Java", "Node.js", "REST APIS",
     "Cloud services", "Kubernetes", "Docker", "GraphQL", "Microservices",
      "gRPC", "Spring Boot", "Flask", "FastAPI", "SQL & NoSQL Databases",
       "Redis", "RabbitMQ", "CI/CD"
    ],
    "Data Engineer": [ "Python", "SQL", "Apache Spark", "Hadoop", "Kafka", "ETL Pipelines", "Airflow", "BigQuery", "Redshift", "Data Warehousing", "Snowflake", "Azure Data Factory", "GCP", "AWS Glue", "DBT"],
    "DevOps Engineer": ["Kubernetes", "Docker", "Terraform", "CI/CD", "AWS", "Azure", "GCP", "Jenkins", "Ansible", "Prometheus", "Grafana", "Helm", "Linux Administration", "Networking", "Site Reliability Engineering (SRE)"],
    "Full Stack Developer": [ "JavaScript", "TypeScript", "React", "Node.js", "Express", "MongoDB", "SQL", "HTML5", "CSS3", "RESTful APIS", "Git", "CI/CD", "Cloud Services", "Responsive Design", "Authentication & Authorization"],
    "Product Manager": ["Product Strategy", "User Research", "Agile Methodologies", "Roadmapping", "Market Analysis", "Stakeholder Management", "Data Analysis", "User Stories", "Product Lifecycle", "AB Testing", "KPI Definition", "Prioritization", "Competitive Analysis", "Customer Journey Mapping"],
...
def setup_agent(config):
    ...
    if st.session_state.resume_agent is None:
        st.session_state.resume_agent = ResumeAnalysisAgent(api_key=config["groq_api_key"], provider="groq", model="llama3-8b-8192")
    else:
        st.session_state.resume_agent.api_key = config["groq_api_key"]
        st.session_state.resume_agent.provider = "groq"

    return st.session_state.resume_agent

def analyze_resume(agent, resume_file, role, custom_jd):
    """Analyze the resume with agent"""
    if not resume_file:
    ...
    try:
        with st.spinner("Analyzing resume... This may take a minute."):
            if custom_jd:
                ...
                result = agent.analyze_resume(resume_file,
                role_requirements = ROLE_REQUIREMENTS[role])
            st.session_state.resume_analyzed = True
            st.session_state.analysis_result = result
            return result
    except Exception as e:
        st.error(f"Error analysing resume: {e}")
        return None
...
def generate_interview_questions(agent, question_type, difficulty, num_questions):
    ...
    try:
        with st.spinner("Generating persinalized interview questions..."):
...
def improve_resume(agent, improvement_areas, target_role):
    """Generating resume improvement suggestion"""
    try:
        with st.spinner("Analysing and generating improvement..."):
...
def cleanup():
    """Clean up resources when app Exist"""
