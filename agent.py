...
from langchain.embeddings import FakeEmbeddings  # Replace with real if Groq has embedding support
...
class ResumeAnalysisAgent:
    def __init__(self, api_key, cutoff_score=75, provider = None, model = "llama3-8b-8192"):
    ...
    def get_llm(self):
        return ChatGroq(groq_api_key=self.api_key, model_name="llama3-8b-8192")
    ...
    def analyze_resume_weaknesses(self):
                ...
                for skill in self.analysis_result.get("missing_skills", [1]):

                    llm =ChatGroq(groq_api_key=self.api_key, model_name="llama3-8b-8192")
                    prompt = f"""
                    ...
                    3.What specific action items would make this skill out?
                    ...
                    "example_additon": " A specific bullet point that could be added to showcase this skill"
                }}
                ...
                try:
                    ...
                    weakness_detail = {
                        "skill": skill,
                        "score": self.analysis_result.get("skill_score", {}).get (skill, 0),
                    ...
                    self.improvement_suggestion[skill] = {
                            "suggestions": weakness_data.get("improvement_suggestions", [1]),
                            "example":weakness_data.get("example_addition", "")
                        }
    ...
    def extract_text_from_file(self, file):
        ...
        return self.extract_text_from_pdf(file) if ext == 'pdf' else self.extract_text_from_txt(file)

    def create_rag_vector_store(self, text):
        chunks = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200).split_text(text)
        embeddings = FakeEmbeddings(size=700)  # Replace with a Groq-compatible embedding if available
        return FAISS.from_texts(chunks, embeddings)

    def create_vector_store(self, text):
        embeddings = FakeEmbeddings(size=700)  # Replace with actual embedding method
        return FAISS.from_texts([text], embeddings)

    def analyze_skill(self, qa_chain, skill):
        query = f"Rate the candidate's proficiency in {skill} from 0 to 10, followed by reasoning."
    ...
    def semantic_skill_analysis(self, resume_text, skills):
        ...
        qa_chain = RetrievalQA.from_chain_type(llm=self.get_llm(), retriever=retriever, return_source_documents=False)
        skill_score, skill_reasoning, missing_skills, total_score = {}, {}, [], 0

        with ThreadPoolExecutor(max_workers=5) as executor:
            results = executor.map(lambda s: self.analyze_skill(qa_chain, s), skills)
        ...
        return {
            ...
            "strengths": [s for s, v in skill_score.items() if v >= 7]
        }
    def extract_skills_from_jd(self, jd_text):
        ...
        try:
            llm=ChatGroq(groq_api_key=self.api_key, model_name="llama3-8b-8192")
            prompt = f"""
            extract a comprehensive list of technicl skills, technologies , and competencies required from this job description .
            format the output as pyrhon list of strings. Only include the list, nothing else.
            Jib Description:
            {jd_text}
            """
    ...
    def generate_interview_questions(self, question_types, difficulty, num_questions):
        """genetrate interview question based on the resume"""
        ...
        try:
            ...
            Format the response as a List of tuple with the question type and the question itself.
            Each tuple should be in thhe format: ("Question Type", "Full question text")"""
    ...
    def improve_resume(self, improvement_areas, target_role =""):
        ...
        try:
            ...
            for area in improvement_areas:
                if area == "skills Highlighted" and self.resume_weaknesses:
                    skill_improvements = {
                        ...
                        if "example" in weakness and weakness["example"]:
                            ...
                            for chunk in resume_chunks:
                                if skill_name.lower() in chunk.lower() or "experience" in chunk.lower():
                                    relevent_chunk = chunk
                                    break
                                if relevent_chunk:
                                    before_after_examples = {"before": relevent_chunk.strip(), "after": relevent_chunk.strip() + "\n" + weakness["example"]}
                ...
                if remaining_areas:
                    ...
                    2. 3-5 specific actionable suggestion
                    3. Where relevant, provide a  before/after example

                    Format the response as a JSON object with improvement areas as keys, each conataining:
                ...
                for area in improvement_areas:
                    if area not in improvements:
                        ...
                        return {area: {"description": "Error generating suggestion", "specific": []} for area in improvement_areas}


    def get_improved_resume(self, target_role="", highlight_skills=""):
        """Generating an improved version of the resume optimized for the job description"""
        if not self.resume_text:
        ...
        try:
            ...
            if highlight_skills:
                if len(highlight_skills)>100:
                    ...
                    try:
                        ...
                        skills_to_highlight = [s.split()  for s in highlight_skills.split(",") if s.strip()]
                else:
                    skills_to_highlight = [s.strip() for s in highlight_skills.split(",") if s.strip()]
            ...
            if self.resume_weaknesses:
                ...
                for weakness in self.resume_weaknesses:
                    ...
                    if 'suggestions' in weakness and weakness['suggestions']:
                        weakness_context += "Suggested improvement:\n"
                        for suggestion in weakness['suggestions']:
                            weakness_context += f" * {suggestion}\n"
                    if 'example' in weakness and weakness['example']:
                        improvement_examples+= f"For {skill_name}: {weakness['example']}\n\n"
            llm = ChatGroq(groq_api_key=self.api_key, temperature=0.7, model_name="llama3-8b-8192")
            jd_context = ""
            if self.jd_text:
            ...
            7. Ensuring all relavent experience is properly emphasized 
    ...
    # def cleanup(self):
    #     pass  # Add temp file cleanup logic if needed

    def cleanup(self):
        """Clean up temporary files"""
        try:
            if hasattr(self, 'resume_file_path') and os.path.exists(self.resume_file_path):
                os.unlike(self.resume_file_path)
            if hasattr(self, 'improved_resume_path') and os.path.exists(self.resume_file_path):
                os.unlike(self.improved_resume_path)
        except Exception as e:
