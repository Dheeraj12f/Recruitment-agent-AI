...
#             logo_based64 = base64.b64encode(img_file.read()).decode()
#             logo_html = f'<img src= "data:image/jpeg;base64,{logo_based64}", alt="Euron Logo" class = "logo-image" style= "max-height: 100px;">'
#     except:
...
#     <p> Smart resume analysis and Interview Preparation System</p>
#     </div>
#     </div>
...
#     }}
#     div.stAlert{{
#         background-color: #4a000 !important;
#         color: white !important;
#     }}
...
#               <p style = "font-size: 0.8rem; color: #666; ">made by DK and AS</p>
#               </div>
#         """, unsafe_allow_html= True)
#         return{
...
#     with col1:
#         role= st.selectbox("Selects the roles you are applying for:", list(role_requirements.keys()))
#     with col2:
#         upload_jd = st.checkbox("Upload the custom job description instead")
#     custom_jd = None
#     if upload_jd:
#         custom_jd_file = st.file_uploader("Upload job Description (PDF and Text)", type=["pdf", "txt"])
#         if custom_jd_file:
...
def role_selection_section(role_requirements):
    ...
    with col1:
        role = st.selectbox("Select the role you are applying for:", list(role_requirements.keys()))
    with col2:
        upload_jd = st.checkbox("Upload a custom job description instead")
...
#     st.markdown('<div class="card">', unsafe_allow_html=True)
#     st.markdown(
#         '<div style="text-align:right; font-size: 0.8rem; color: #888; margin-bottom:10px;">Powered by Recruitment Agnet</div>', unsafe_allow_html=True
#     )
...
#     with col2:
#         if selected:
#             st.markdown("<h2 style = 'color:#4CAF50;'> Congratulation! you have been shortlisted.</h2>", unsafe_allow_html=True )
#         else:
#             st.markdown("<h2 style = 'color:#4CAF50;'> Unfortunately, you were not selected.</h2>", unsafe_allow_html=True )
#             st.write(analysis_result.get('reasoning', ''))
#     st.markdown('<hr>', unsafe_allow_html=True)
#     st.markdown('<div class = "strength-improvements">', unsafe_allow_html=True)

#     st.markdown('<div>', unsafe_allow_html = True)
#     st.suubheader(" Strengths")
#     strengths = analysis_result.get("strength", [])
#     if strengths:
#         for skill in strengths:
#             st.markdown(f'<div class = "skill-tag">{skill}({skill_scores.get(skill,"N/A")}/10)</div>', unsafe_allow_html = True)
#         else:
#             st.write("No notable strenght identified.")
#         st.markdown('</div>', unsafe_allow_html = True)
...
#     with col2:
#         report_content = f"""
#         #Recruitment -Result Analysis Report
#         ##Overall Score : {overall_score}/100
#         Status: {"ShortListed" if selected else "Not Selected"}

#         ##Analysis Reasoning
#         {analysis_result.get('resoning', 'No reasoning provided.')}
...
#             skill_name = weakness.get('skill', '')
#             score = weakness.get('score', 0)
#             detail = weakness.get('detail', 'No specific detials provided.')

#             #Clean Json formatting if it appear in the text
#             if detail.startswith('```json') or '{' in detail:
#                 detail  = "The resume lack example of this skill."
...
#             if 'suggestions' in weakness and weakness['suggestion']:
...
#                 report_b64 = base64.b64decode(report_content.encode()).decode()
#                 href = f'<a class = "download-btn" href = "data:text/plain;base64, {report_b64}" download= "euron_resume_analysis.text"> Download Analysises Report</a>'
...
#         example_questions = [
#             "What is the Candidate most recent role?", "How many Year of experience does the candidate have with python", "what educational qualification does candidate have?","what are the candidate key achievement?", "has the candidate manage teams Before>", "What projects has the candidate work on ?", "Does candidate have exxperiience with cloud techno"
#         ]
...
def create_score_pie_chart(score):
    ...
    sizes = [score, 100 - score]
    labels = ['', '']
    colors = ["#4CAF50", "#333333"] if score >= 75 else ["#d32f2f", "#333333"]  # FIX: Color reflects pass/fail
    explode = (0.05, 0)
    wedges, texts = ax.pie(
        sizes,
        labels=labels,
...
def display_analysis_results(analysis_result):
    ...
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div style="text-align:right; font-size: 0.8rem; color: #888; margin-bottom:10px;">Powered by Recruitment Agent</div>', unsafe_allow_html=True)
    ...
    if missing_skills:
        for skill in missing_skills:
            st.markdown(f'<div class="skill-tag missing">{skill} ({skill_scores.get(skill, "N/A")}/10)</div>', unsafe_allow_html=True)  # FIX: closing tag
    else:
        st.write("No significant areas for improvement.")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
"""
        ...
        report_content += "\n---\nAnalysis Provided by Recruitment Agent"
        report_b64 = base64.b64encode(report_content.encode()).decode()  # FIX: Use encode not decode
        href = f'<a class="download-btn" href="data:text/plain;base64,{report_b64}" download="resume_analysis_report.txt">Download Analysis Report</a>'
        st.markdown(href, unsafe_allow_html=True)
...
#     if not has_resume:
#         st.warning("Please Upload and Aanalyse the resume first.")
#         return
#     st.markdown('<div class = "card">', unsafe_allow_html = True)
#     col1, col2 = st.columns(2)
#     with col1:
#         question_types = st.multiselect(
#             "Select question types:", ["Basic", "Technical", "Experience", "Scenario", "Coding", "Behavioral"], default = ["Basic", "Technical"])
...
#     if not has_resume:
#         st.warning("please upload and analse the resume first :")
#         return
#     st.markdown('<div class = "card">', unsafe_allow_html = True)
#     improvement_areas = st.multiselect(
        
#     "Select ares to improve:",
...
#             with st.spinner("Analyse and generating improvement..."):
...
#                 for area, suggestions in improvements.item():
...
#                     download_content += "### Specific Suggestion\n\n"
#                     for i, suggestion in enumerate(suggestions["specific"]):
...
#                 href = f'<a class = "download-btn" href="data:text/markdown;base64,{b64}" download="euron_resume_improvement.md"> Download all Suggestions</a>'
#                 st.markdown('</div>', unsafe_allow_html = True)
#     st.markdown('</div>', unsafe_allow_html = True)
...
#     target_role = st.text_input("Target_role:", placeholder= "e.g., Senior Software Engineer")
...
#     st.markdown('</div>', unsafe_allow_html = True)
# def create_tabs():
#     return st.tabs(["Resume Analysis",
#                      "Resume Q&A",
#                      "Interview Questions",
#                      "Resume Improvement",
#                      "Improved Resume"
...
def interview_questions_section(has_resume, generate_questions_func=None):
    if not has_resume:
        st.warning("Please upload and analyze the resume first.")
        return
        ...
        if get_improved_resume_func:
            with st.spinner("Creating improved resume..."):
                ...
                with col1:
                    resume_bytes = improved_resume.encode()
                    b64 = base64.b64encode(resume_bytes).decode()
                    href = f'<a class="download-btn" href="data:file/txt;base64,{b64}" download="euron_improved_resume.txt">Download as Text</a>'
                    st.markdown(href, unsafe_allow_html=True)
